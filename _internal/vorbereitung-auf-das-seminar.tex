...
Anaconda ist eine Python-Distribution, die die Integrated Development Environment (IDE) \emph{Jupyter Notebook}, welche im Seminar verwendet wird, bereits mit ausliefert.
...
strukturiert Anaconda Bibliotheken standardmäßig in Umgebungen (eng. Environments).
...
Danach startet ein Klicken auf \texttt{Launch} die IDE im Browser.
...
\begin{figure}[h]
  ...
  \caption{Aus dem Anaconda Navigator kann ein Jupyter Notebook gleich in der richtigen Umgebung gestartet werden.}%
\label{fig:start-ide}
\end{figure}

\section{Erste Schritte mit der IDE}

Wenn Sie die Seminarmaterialien wie unter Abschnitt \enquote{\nameref{sec:git-clone}} angegeben unterhalb des Ordners \texttt{Eigene Dateien} abgelegt haben, 
können Sie in der IDE Jupyter Notebook zum Ordner \texttt{00 installations-check} navigieren und dort überprüfen, ob Sie alle Zellen ausführen können.
Falls es Fehlermeldungen gibt, melden Sie sich gerne bereits im Vorwege an die Seminarleitung.
Falls Sie noch keine Erfahrungen mit Python haben und Sie Interesse haben, sich in diese Sprache einzudenken,
können Sie sich die Einführungsmaterialien unter \texttt{01 einfuehrung-in-python} anschauen.
Diese Materialien sind nicht verpflichtend.
